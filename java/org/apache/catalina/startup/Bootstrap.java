...
public final class Bootstrap {
    ...
    public void init() throws Exception {
        ...
        // 创建三个 classLoader     commonLoader、catalinaLoader、sharedLoader
        initClassLoaders();

        // 当前线程设置上下文的 classLoader（contextClassLoader） 为 catalinaLoader
        ...
        // 用 catalinaLoader 加载并实例化 org.apache.catalina.startup.Catalina
        ...
        // 设置 Catalina 的 parentClassLoader = sharedLoader
...
//        startupInstance: {"wait": false, "configFile":"conf/server.xml", "parentClassLoader":"URLClassLoader的实例", ...}
   ...
   /**
    ...
    public void setAwait(boolean await)
        ...
        // 这里可以去看 Catalina.setAwait(boolean b) 的实现逻辑
        // 就是把变量 await 设置为指定的值
    ...
    public static void main(String args[]) {
        ...
        synchronized (daemonLock) {
            if (daemon == null) {
                ...
                // 创建一个 Bootstrap 类的实例，这里不能直接赋值给 daemon，执行完 init() 方法之后才可以
                Bootstrap bootstrap = new Bootstrap();
                try {
                    // 初始化 classloader，并初始化 catalina
                    bootstrap.init();
                } catch (Throwable t) {
                    // 初始化异常，处理异常后，直接结束
                    handleThrowable(t);
        ...
        try {
            ...
            if (command.equals("startd")) {
                ...
                // 以下三个方法都是通过反射调用 catalina 方法（可以进入方法看下，都是使用的反射）
                // 思考：这里为什么要用反射来处理，而不是直接使用 catalina 的实例？
